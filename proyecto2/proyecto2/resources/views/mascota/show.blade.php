<x-app-layout>
    ...
    <div class="container px-5">
        ...
        <div class="mb-5">
            <form action="{{ route('usuarios.seleccionar-mascota', ['usuario' => $usuario]) }}" method="POST"
                id="formSeleccionarMascota">
                @csrf
                ...
                <select name="mascota_id" id="mascota" class="text-black" onchange="this.form.submit()">
                    @foreach ($mascotas as $m)
                        <option value="{{ $m->id }}" @if ($m->id == $mascota->id) selected @endif>
                            {{ $m->nombre }}</option>
                    @endforeach
                </select>
            </form>
                    ...
                    @if (auth()->user()->hasRole('employee'))
                        <div class="d-flex align-items-center justify-content-between mb-4">
                            ...
                            <button id="mostrarFormulario"
                                class="btn bg-gradient-primary-to-secondary text-white px-4 py-3">
                                <span id="add-appointment">{{ __('Agregar Cita') }}</span>
                            </button>
                        </div>
                    @endif
                    <div id="formularioCita"
                        class="hidden p-6 bg-gray-800 dark:bg-gray-700 border-b border-gray-600 dark:border-gray-600">
                        <section class="py-5">
                            <div class="container px-5">
                                <!-- Contact form-->
                                <div class="rounded-4 py-5 px-4 px-md-5">
                                    <div class="text-center mb-5">
                                        ...
                                        <p class="lead fw-normal text-muted mb-0"><span id="explain-reason">Explica
                                                detenidamente el motivo de la Cita</span></p>
                                    </div>

                                    <h3 class="text-lg font-semibold mb-4 text-white" id="adddd-appointment">
                                        {{ __('Agregar Cita') }}</h3>
                                    ...
                                    <form action="{{ route('citas.store') }}" method="POST">
                                        ...
                                        <div class="mb-4">
                                            <label for="fecha" class="block text-sm font-medium text-gray-300"
                                                id="date">{{ __('Fecha de la Cita:') }}</label>
                                            <input type="date" name="fecha" id="fecha"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-800 text-black"
                                                required>
                                            @error('fecha')
                                        ...
                                        <div class="mb-4">
                                            <label for="motivo"
                                                class="block text-sm font-medium text-gray-300">{{ __('Motivo de la Cita:') }}</label>
                                        ...
                                        <div class="mb-4">
                                            <label for="diagnostico"
                                                class="block text-sm font-medium text-gray-300">{{ __('Diagnóstico:') }}</label>
                                        ...
                                        <div class="mb-4">
                                            <label for="tratamiento"
                                                class="block text-sm font-medium text-gray-300">{{ __('Tratamiento:') }}</label>
                                        ...
                                        <div>
                                            <button type="submit"
                                                class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-nonefocus:border-indigo-700 focus:ring focus:ring-indigo-300 transition">
                                ...
                                <div class="row align-items-center gx-5">
                                    <div class="col text-center text-lg-start mb-4 mb-lg-0">
                                        <div class=" p-4 rounded-4">
                                            ...
                                            <div class="small text-black"><span id="pet-name">Nombre de la
                                                    mascota:</span> {{ $cita->mascota->nombre }}</div>
                                            <div class="small text-muted"><span id="pet-type">Tipo de mascota:</span>
                                                {{ $cita->mascota->tipo }}</div>
                                            <div class="small text-muted"><span id="appointment-reason">Motivo de la
                                                    Cita:</span> {{ $cita->motivo }}</div>
                                            <div class="small text-muted"><span id="diagnosis">Diagnóstico:</span>
                                                {{ $cita->diagnostico }}</div>
                                            <div class="small text-muted"><span id="treatment">Tratamiento:</span>
                                                {{ $cita->tratamiento }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
